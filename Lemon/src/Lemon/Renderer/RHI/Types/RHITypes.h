...
/// @brief Input element types. Represents the layout type that can be passed to
/// a vertex layout
///
...
constexpr u32 operator&(ShaderStage a, ShaderStage b) {
...
enum class PrimitiveTopology : u32 {
    /// @brief Specifies a list of triangles with defined vertices, each 3
    /// vertices form a single
    /// triangle
    TriangleList,
    /// @brief Specifies a <a
    /// href="https://docs.safe.com/fme/html/FME-Form-Documentation/FME-Form/!FME_Geometry/IFMETriangleStrip.htm">
    ...
    TriangleStrip,
    /// @brief Specifies a <a
    /// href="https://docs.safe.com/fme/html/FME-Form-Documentation/FME-Form/!FME_Geometry/IFMETriangleFan.htm">
    /// Triangle Fan</a> sharing a center vertex. The first 3 vertices define the
    /// first triangle.
    /// Subsequent vertices form new triangles sharing the center and the last
    /// triangle.
    /// This replaces loop-generated polygon indices (although not frequently
    /// used)
    TriangleFan,

    /// @brief Specifies a list of lines. Lines do not share vertices, each 2 new
    /// vertices form a
    /// new line
    LineList,
    /// @brief Specifies a set of lines. Each new vertex connects with the last
    /// one
    LineStrip,
...
enum class RootParamType : u32 {
    ...
    ConstantBufferView,  /// DX12: root CBV        | Vulkan:
                         /// VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER
    ShaderResourceView,  /// DX12: root SRV        | Vulkan:
                         /// VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE
    UnorderedAccessView, /// DX12: root UAV        | Vulkan:
                         /// VK_DESCRIPTOR_TYPE_STORAGE_IMAGE
};

/// @brief Root parameters / Uniform / Push constants / Descriptor bindings
///
/// @code maps to CD3DX12_ROOT_PARAMETER / VkPushConstantRange +
...
struct RootParameter {
    ...
    RootParameter(RootParamType type, u32 count, u32 shaderReg, u32 space, ShaderStage visibility)
        : type(type), count(count), shaderReg(shaderReg), space(space), visibility(visibility) {}
};
...
///
/// @code maps to D3D12_DEPTH_STENCIL_DESC /
/// VkPipelineDepthStencilStateCreateInfo @endcode
...
/// @brief Blend state
///
/// @code maps to D3D12_BLEND_DESC / VkPipelineColorBlendStateCreateInfo
/// @endcode
struct BlendState {
    bool blendEnable = false;
    BlendType srcBlend = BlendType::SrcAlpha;
    BlendType dstBlend = BlendType::InvSrcAlpha;
    BlendOp blendOp = BlendOp::Add;
    BlendType srcBlendAlpha = BlendType::One;
    BlendType dstBlendAlpha = BlendType::Zero;
...
/// @code maps to D3D12_TEXTURE_ADDRESS_MODE @endcode
...
/// @code maps to D3D12_FILTER_MIN_MAG_* @endcode
enum class Filter {
    Linear,
    Nearest
};

/// @brief Static sampler description
///
/// @code maps to D3D12_STATIC_SAMPLER_DESC @endcode
struct StaticSamplerDesc {
    ...
    StaticSamplerDesc(Filter filter, AddressMode addressU, AddressMode addressV, AddressMode addressW,
                      u32 shaderRegister, u32 space, ShaderStage visibility)
        : filter(filter), addressU(addressU), addressV(addressV), addressW(addressW),
          shaderRegister(shaderRegister), space(space), visibility(visibility) {}
    
    StaticSamplerDesc(Filter filter, AddressMode addressMode, u32 shaderRegister, u32 space, ShaderStage visibility)
        : filter(filter), addressU(addressMode), addressV(addressMode), addressW(addressMode),
          shaderRegister(shaderRegister), space(space), visibility(visibility) {}
};
...
enum class BufferUsage { Vertex, Index, Uniform, Storage, Indirect, Staging };
